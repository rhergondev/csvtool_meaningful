...
* Docker Engine installed (Follow official Docker guide for Ubuntu: https://docs.docker.com/engine/install/ubuntu/ or your expecific OS: https://docs.docker.com/desktop/)
* User added to the `docker` group (The docker group is an autogenerated group during Docker installation that allows to execute docker commands without Superuser access) or use `sudo` for Docker commands (as shown below).
